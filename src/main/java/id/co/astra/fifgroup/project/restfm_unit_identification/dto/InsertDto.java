package id.co.astra.fifgroup.project.restfm_unit_identification.dto;

import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.Getter;
import lombok.Setter;

import java.time.LocalDate;

@Getter
@Setter
public class InsertDto {

    private  String suplCode;
    private  String suplBranchId;
    private  String suplName;
    private  String suplAddress1;
    private  String suplAddress2;
    private  String suplCity;
    private  String suplProvince;
    private  String suplKecamatan;
    private  String suplKelurahan;
    private  String suplZipCode;
    private  String suplTelp1;
    private  String suplFax;
    private  String suplEmail;
    private  String suplNpwp;
    private  String suplPkp;
    private  String pcSubType;
    private  String suplTglPkp;
    private  String suplTaxCode;
    private  String suplUserApproved;
    private  String suplDateApproved;
    private  String suplStatus;
    private  String suplType;
    private  String suplCreatedBy;
    private  String suplCreatedDate;
    private  String suplCompType;
    private  String suplNpwpName;
    private  String suplNpwpAddress;
    private  String suplNonPph;
    private  String suplNonPphStartdate;
    private  String suplNonPphEnddate;
    private  String suplNonPphDocno;
    private  String suplSiup;
    private  String suplTdp;
    private  String suplTdpExpdate;
    private  String suplBankId;
    private  String suplBankName;
    private  String suplAccountNo;
    private  String suplAccountName;
    private  String suplAccStatus;
    private  String personalId;
    private  String personalFullName;
    private  String personalSertProfesi;
    private  String personalSertProfesiDate;
    private  String personalBirtPlace;
    private  String personalBirtDate;
    private  String personalAddr1;
    private  String personalCity;
    private  String personalKecamatan;
    private  String personalKelurahan;
    private  String personalRtRw;
    private  String personalZipCode;
    private  String personalReligion;
    private  String personalSex;
    private  String personalMaritalSts;
    private  String lastEducationLvl;
    private  String personalMobilephone;
    private  String personalEmail;
    private  String personalIdIndentifikasi;
    private  String personalIdAddr1;
    private  String personalIdCity;
    private  String personalIdZipCode;
    private  String personalIdEmcName;
    private  String personalIdEmcAddr1;
    private  String personalIdEmcPhone;

}
