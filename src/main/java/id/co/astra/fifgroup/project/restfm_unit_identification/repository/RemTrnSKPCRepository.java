...
public interface RemTrnSKPCRepository extends JpaRepository<RemTrnSkpc, String> {
    ...
    @Query (value = "SELECT * FROM REM_TRN_SKPC WHERE PC = :pc AND CONTRACT_NO = :contractNo AND SKPC_NO = :skpcNo AND to_char(SKPC_DATE,'DD-MM-YYYY') =     :skpcDate", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM REM_TRN_SKPC WHERE CONTRACT_NO = :contractNo and PC = :pc and STATUS_SKPC = 'AC' and SKPC_NO = :skpcNo", nativeQuery = true)
    List<RemTrnSkpc> skpcService(@Param("contractNo") String contractNo, @Param("pc") String pc, @Param("skpcNo") String skpcNo);

    @Transactional
    @Modifying
    @Query(value = "UPDATE REM_TRN_SKPC SET STATUS_SKPC = 'CL', SKPC_EXPIRED_DATE = SYSDATE, LASTUPDATE_BY = 'MOTIF', LASTUPDATE_TIMESTAMP = SYSDATE WHERE CONTRACT_NO = :contractNo and PC = :pc and STATUS_SKPC = 'AC' and SKPC_NO = :skpcNo", nativeQuery = true)
    void skpcServiceUpdate (@Param("contractNo") String contractNo, @Param("pc") String pc, @Param("skpcNo") String skpcNo);

    @Query(value = "SELECT * FROM REM_TRN_SKPC WHERE SKPC_NO = :skpcNo", nativeQuery = true)
