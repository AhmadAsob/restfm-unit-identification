...
public interface RemTrnMotifPuRepository extends JpaRepository<RemTrnMotifPu, String> {
    ...
    @Query(value = "SELECT * FROM ACCTMGMT.REM_TRN_MOTIF_PU WHERE PC_ID = :pcId AND CONTRACT_NO = :contractNo", nativeQuery = true)
    List<RemTrnMotifPu> findData(@Param("pcId") String pcId, @Param("contractNo") String contractNo);


    @Query(value = "select * from ACCTMGMT.REM_TRN_MOTIF_PU where CONTRACT_NO = :contractNo AND BAPKP_NO = :bapkpNo ", nativeQuery = true)
    ...
    @Query(value = "select * from ACCTMGMT.REM_TRN_MOTIF_PU where PC_ID = :pcId AND CONTRACT_NO = :contractNo AND BAPKP_NO = :bapkpNo AND STATUS = :status ", nativeQuery = true)
